...
public class OrderLevelPromotionHandler extends PromotionHandler {

    private final PromotionPipeline pipeline;

    public OrderLevelPromotionHandler(PromotionPipeline pipeline) {
        this.pipeline = pipeline;
    }

    @Override
    protected boolean supports(PromotionRule rule) {
        return rule.type() == PromotionType.PERCENTAGE_ORDER
                || rule.type() == PromotionType.VIP_CUSTOMER;
    }
